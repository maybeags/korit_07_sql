...
예제 : users에서 city 별 회원 수를 카운트 하고, 회원 수가 3 명 이상인 도시명과 회원 수를 출력할 것(단, 회원 수를 기준으로 내림 차순 정렬) - 정 안떠오르면 일단 GROUP BY / HAVING으로 작성해보셔도 됩니다.
```sql
...
git add .
git commit -m "feat: 20250917 SQL reading part"
git push
